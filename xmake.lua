...
rule("lce.core", function()
    on_load(function(target)
        ...
        -- Pin the core: the adapter builds against 0.5.0+ (the boundary
        -- contract: the snapshot API, FromConfiguration tuning, the
        -- seeded Rng, and MemoryEvent's world day). A stale or missing
        -- checkout fails loudly instead of silently building against the
        -- wrong API.
        ...
        if not (major and minor) or (tonumber(major) == 0 and tonumber(minor) < 5) then
            raise(
                "LCE core at %s is %s.%s — this adapter requires 0.5.0+ "
                    .. "(the boundary contract). Update the checkout or set LCE_CORE_PATH.",
                corePath,
                major or "?",
                minor or "?")
        end
