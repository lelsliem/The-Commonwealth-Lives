...
## 0062 — The dialog swap: the game's own lines, voice-aware silence (0.8.7, 2026-08-15)

The drafted one-liners were content, and content should be the game's
own words — the subtitle layer's whole promise is that the lines the
player reads will one day *speak* in the game's real voices, so the
text had to come from the game's own recordings. The voice-bank survey
(Docs/Dialogue/LineCatalog.md) extracted and classified the game's
settler bank (~17,500 files across the 8 settler voices), the 4 guard
voices, Gen3 synths, and children, then curated every pool to the
game's real lines with per-voice coverage recorded (`(8v)` = all 8
settler voices recorded it). The swap was pure data: `src/Dialogue.h`
defaults + the INI `dialogue.*` keys, one test updated (DialogueTest
asserted the old drafted ramp lines; it now asserts the catalog's
signature — the guard escalation — plus the size floor), 27/27 green,
deployed and verified in-game.

**The comma gotcha.** The INI list parser splits on commas, and many
curated lines contain them ("Hey, how's it going?"). Mid-line commas
become em-dashes — the convention the old pool already used — so the
text reads the same and the parser stays happy. Documented in the INI
header so future curators don't reintroduce commas.

**The Realistic Conversations compatibility (0.8.7).** The 2018 ESP of
that name (flagged broken on the Next-Gen update) drives the game's
NPC-to-NPC *voiced* ambient chatter through 33 GMST overrides. Rather
than xedit-patch the ESP, the adapter re-delivers the overrides as a
tuning file (`Realistic Conversations.ini` next to the DLL) and
applies them to the game's own `GameSettingCollection` at load — no
ESP, no load-order slot, no patching. A missing file is the off
switch; a setting the game no longer has is skipped, never fatal. Our
sim's subtitle lines are a separate text channel that never touches
these settings — the two layers coexist without stepping on each
other.

**Voice-aware picking is the audio layer's rule, decided now.** The
game resolves audio by voice type: a MaleOld settler can only speak
lines recorded in MaleOld. The next stone (the audio trigger probe, within 0.8.7) triggers the
actual .fuz recordings, and the design rule is settled: a line only
speaks if the speaker's voice bank recorded it — each catalog line's
`(nv)` coverage is exactly that data. When the speaker's voice has no
recording of the line, they stay mute: captions only, never a
wrong-voice line. If the game doesn't provide the line at all,
nothing can be done — silence is honest.
## 0063 — Voice-aware picking: ground truth from the BA2, not the catalog's counts (0.8.7, 2026-08-15)

The dialog swap (0062) put the game's own lines in the subtitle layer; the
next stone makes them *speak*. The design rule was decided with the
author up front: a line only plays if the speaker's voice bank
recorded it — the game resolves audio by voice type, so a voice can
never say a line its bank lacks; when no line exists for that voice,
the mind stays mute (captions only); if the game doesn't provide the
line at all, nothing can be done. The engine was asked first: it
needs nothing — the engine is pure simulation with zero game types
(no Actor, no forms, no audio), and both the voice data (the actor's
NPC voiceType) and the audio trigger are game-facing, which the
adapter already owns.

**The picker is built on ground truth, not the catalog's `(nv)`
counts.** The LineCatalog records how many of the 8 settler voices
recorded each line, but not *which* — and the picker must know
exactly which. The game's own `Fallout4 - Voices.ba2` name table is
the truth: each entry is `Sound\Voice\Fallout4.esm\<VoiceType>\<FORMID>_<idx>.fuz`,
so the existing extraction tool (settler_catalog.py) yields a
formid → voice-set map, and every curated line's coverage becomes its
actual recorded voices. The survey corrected two assumptions: "Last
warning." (catalog 4v, assumed guard-only) is recorded by all 8
settler voices *and* the guards — 0x0FFF; "No loitering." is
genuinely guard-only — 0x0F00. The harness pins both.

**The shape.** `Dialogue::Voice` (8 settler + 4 guard + 2 child, as
bits), a coverage table keyed by normalized line text (the INI's
comma-safe em-dash form meets the catalog's original commas), and
`PickForVoice` — deterministic like Pick, seeding the voice too, and
returning empty (mute) when the speaker's voice has nothing in the
pool. A custom INI line with no catalog entry has coverage 0:
caption-only, never wrong-voice audio. Adapter-side, `VoiceOfForm`
reads the actor's NPC voiceType editor id (verified against the
ESM's VTYP table — all 14 names exist) and `Say` uses PickForVoice
when it resolves, logging `[voice]` vs `[cap]` per line so the
in-game log proves which path ran.

**What this deliberately does NOT do — the audio trigger.** Making
the game actually play a line's .fuz on an actor is the remaining
probe: the vendored commonlibf4 exposes no direct Say wrapper, so
the probe tests the ProcessGreet / dialogue-package route in-game.
The picker is built first precisely because it is pure, harness-
testable, and makes the trigger safe: when the trigger lands, it can
only ever play lines the speaker's voice recorded.
## 0063 addendum — the ghoul bank: shared lines, not a new register (0.8.7)

The field test exposed the last caption-only group: ghoul settlers.
The diagnostic (`vfx: voice editor id 'FemaleGhoul' is not a settler/
guard/child bank`) showed ghouls resolve to their own voice types, so
every ghoul line was `[cap]`. The ghoul banks were then surveyed
(FemaleGhoul 1,416 files, MaleGhoul 1,442 — the settler census never
includes them because their NPC bases carry their own VTYP).

**The finding, two halves:** (1) the ghoul bank has no genuine
ghoul-exclusive generic register — its own lines are nearly all
player-directed (chems, quests, the Slog's tarberry/Daisy/Wiseman
business, kidnapping cries); (2) it recorded the **same 8v generic
lines as the settlers** — 52 of the settler pool's 8v lines, same
formids, verified in the Voices BA2. So the ghoul pool is the shared
lines, honestly: the coverage table now carries ghoul bits (14–15)
on 64 curated lines, and a ghoul voice speaks them from the same
table the settlers use. A ghoul can say "Looking to buy?"; it still
can't say "No loitering." (guard-only) or "Hello." (child-only). No
new lines were authored — inventing ghoul dialogue would have meant
recording it, which this mod cannot do. The auto-classified 313-line
ghoul catalog lives in the Not-for-github tools folder as the
evidence for what was cut.

**Why shared lines are honest here:** the game resolves audio by
voice type against the formid — FemaleGhoul literally has the .fuz
