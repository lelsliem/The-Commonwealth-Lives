...
| 0.6.0 | Life & Emergent Quests ("The Commonwealth Remembers") | ⬜ in progress — **Stone 1 verified in-game (2026-08-10): the world keeps its books — a pure `Lifecycle::Diff` (arrivals / deaths / departures) on the tick's one-second census; one `SeedMind` path for the wake seed and mid-session arrivals; deaths clean the mind's book entries and write the death fact to every survivor (the core gained `InteractionKind::Death`); faction-left departures get a goodbye. Verified: a real kill books confirming → died one second apart, the dead never restore and never ghost-walk, a fresh new game books zero false deaths, already-dead settlers from old saves are parked and never booked (three hardenings — the 3D gate, two-pass confirmation, the alive-first rule; ADR-0011). Stone 2 VERIFIED in-game (2026-08-11): bonds — the adapter owns an EventBus (Request A, stone 08) subscribed to `RelationshipChangedEvent`; `sim.bond.threshold.*` from the INI; a mutual + sticky derivation in `Bonds.h`; co-save v5; and the two in-game discoveries that made it real — the buyer's half of a trade is `Remember({keeper, Social})` and the living drift clock (`sim.drift.rate = 0.0002`). ADR-0012. Stone 3 VERIFIED in-game 2026-08-11: households — the Spouse bond forms a household (`Households.h`, pure): two pouches merge into one shared wallet, the family bench feeds the keeper's spouse without exchange, a dead spouse's pouch passes to the widow(er), one-pouch enforced silently on restore — derived state, no record bump (ADR-0013). In-game: a real marriage emerged from shared meals (`0x2a8a7` + keeper `0x50976`), survived save/load, and the family bench fed the spouse for free. Stone 3.5 built + tested (13/13 suites at the time): the sleep cycle — `RestRecovery` restores Fatigue, Safety, and Comfort before `Update` (ADR-0014), closed by the review pass (ADR-0015: needs-keyed rescue of parked minds, arrival ends the session, `sim.walk.cap` INI-tunable, the launch banner prints every live need rate). 2026-08-11 build pass (ADR-0017) — Stones 4–6 built, 16/16 suites green, in-game verification pending: Stone 3.75 the meal-cadence hold (`Movement::HoldPlace` — Rest and Explore now execute in-game as a commanded hold, one per mind per 10 s, so a fed mind rests at its bench instead of drifting away; walk/rest/explore are all real game commands now); Stone 4 gossip (`Gossip.h` — a bond, feud, or death writes a fact to every mind of the settlement, wired into `OnBondChange` and the death bookkeeping); Stone 5 the arcs (`Arcs.h` — a settlement that has heard of an enemy pair tries to cool it once per day, a liked mediator warms the pair a step toward zero; a loved death drains the bereaved's Social at `sim.arc.grief.decay`, derived from persisted components so grief survives save/load); Stone 6 birth (experimental, `sim.birth.enabled` off by default — a spouse household's sim-only child, fed by the household, bonded to both parents, living in the co-save like any mind). No co-save bump: a child rides existing components; gossip and arcs are derived (ADR-0017). Next: in-game verification of Stones 3.75–6** |
