...
| 0.6.0 | Life & Emergent Quests ("The Commonwealth Remembers") | ⬜ in progress — **Stone 1 verified in-game (2026-08-10): the world keeps its books — a pure `Lifecycle::Diff` (arrivals / deaths / departures) on the tick's one-second census; one `SeedMind` path for the wake seed and mid-session arrivals; deaths clean the mind's book entries and write the death fact to every survivor (the core gained `InteractionKind::Death`); faction-left departures get a goodbye. Verified: a real kill books confirming → died one second apart, the dead never restore and never ghost-walk, a fresh new game books zero false deaths, already-dead settlers from old saves are parked and never booked (three hardenings — the 3D gate, two-pass confirmation, the alive-first rule; ADR-0011). Stone 2 VERIFIED in-game (2026-08-11): bonds — the adapter owns an EventBus (Request A, stone 08) subscribed to `RelationshipChangedEvent`; `sim.bond.threshold.*` from the INI (adapter defaults when the file names none); a mutual + sticky derivation in `Bonds.h` (formation at the line, dissolution halfway back — drift is quiet, so the 1-second `ReconcileBonds` pass is the dissolve net and the event channel is the instant formation); co-save v5 (form pair, kind, since-day) so a spouse survives reload; and the two in-game discoveries that made it real — the buyer's half of a trade is `Remember({keeper, Social})` (Trade builds trust, never disposition) and the living drift clock (`sim.drift.rate = 0.0002`, ~1 h half-life, injected as the adapter default). ADR-0012. Stone 3 VERIFIED in-game 2026-08-11: households — the Spouse bond forms a household (`Households.h`, pure): two pouches merge into one shared wallet (split on dissolve), the family bench feeds the keeper's spouse without exchange, `PouchOf` resolves the wallet on both sides of the bench, a dead spouse's pouch passes to the widow(er), and the one-pouch invariant is enforced silently on restore — households are derived state (marriage rides the v5 bond map, the wallet rides the `CapPouch` component), no record bump (ADR-0013). In-game: a real marriage emerged from shared meals (`0x2a8a7` + keeper `0x50976`, friend → sweetheart → spouse across sessions), the spouse survived save/load (`bonds: 23 bonds restored`, household reformed silently), and the family bench fed the spouse for free (`fed from the household's meal`). Stone 3.5 built + tested 2026-08-11 (13/13 suites), in-game verification pending: the sleep cycle — the 24h-market test exposed a parked world (a fed mind with drained Fatigue decides Rest, a table slot that does nothing, forever; only Hunger was ever restored, on    the meal), so `RestRecovery` restores the needs a nap fixes — Fatigue, Safety, and Comfort — before `Update` at `sim.rest.recovery` (default 0.2/s), closing the loop eat → rest → recover → walk → eat so the same pair can finally share repeated meals and bond (ADR-0014). Review pass 2026-08-11 (ADR-0015): the recovery reads the needs, not just the Rest intent — a Safety-drained mind the engine would otherwise silence (nullopt, nothing to flee) is rescued — and the walk layer was fixed (arrival ends the session, a Reached session is a new trip; before that the finished trip held a walk slot for the 120 s timeout and swallowed the re-walk — 7 trades, 7 buyers, zero repeats); the walk cap is INI-tunable (`sim.walk.cap`) and the launch banner prints every live need rate. Remaining stones planned in `Docs/Design/Life.md` (gossip, arcs, births)** |
...
`xmake run TheLivingCommonwealth.Tests`). **13/13 suites green.**
